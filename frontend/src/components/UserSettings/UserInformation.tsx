...
import {
  ...
  UsersService,
  type UserUpdateMe,
  TenantPublic // Import TenantPublic if needed for type safety
} from "@/client";
...
const UserInformation = () => {
  ...
  const {
    ...
    values: currentUser ? { // Ensure form updates if currentUser reloads
      ...
      >
        ...
        <Field label="Tenant" mt={4}>
            {isLoadingTenant ? (
                ...
                <Text fontSize="md" py={2} color="red.500">Error loading tenant</Text>
            ) : (
        ...
        <Field label="Full name" mt={4}> {/* Added mt={4} for spacing */}
          {editMode ? (
            <Input
