...
import type { TimeSeriesData } from "@/client" // Adjust path
...
const EnergyDashboard = ({
  ...
  const chartOptions = useMemo((): Highcharts.Options | null => {
    if (
      !apiResponse ||
      !apiResponse.series ||
      apiResponse.series.length === 0
    ) {
      return null
    }

    // Map API series directly to Highcharts series
    const highchartsSeries = apiResponse.series.map(
      (seriesData: TimeSeriesData, index: number) => {
        ...
        const transparentColor = Highcharts.color(stopColorString)
          .setOpacity(0)
          .get("rgba") as string
    ...
    const optionsTemplate: Highcharts.Options = {
      ...
      tooltip: {
        ...
        pointFormat:
          '<span style="color:{point.color}">\u25CF</span> {series.name}: <b>{point.y:.2f} kWh</b><br/>',
      },
  ...
  return (
    ...
    >
      ...
      <Flex justify="space-between" align="center" mb={4} wrap="wrap" gap={2}>
        <ButtonGroup size="sm" variant="outline" attached>
          <Button
            ...
            data-active={timeRange === "1D"}
          >
            Day
          </Button>
          <Button
            onClick={() => handleTimeRangeChange("1W")}
            data-active={timeRange === "1W"}
          >
            Week
          </Button>
          <Button
            onClick={() => handleTimeRangeChange("1M")}
            data-active={timeRange === "1M"}
          >
            Month
          </Button>
          <Button
            onClick={() => handleTimeRangeChange("1Y")}
            data-active={timeRange === "1Y"}
          >
            Year
          </Button>
          <Button
            onClick={() => handleTimeRangeChange("All")}
            data-active={timeRange === "All"}
          >
            Lifetime
          </Button>
        </ButtonGroup>
      ...
      <Box flex="1" h="0" minH="300px">
        {" "}
        {/* Use flex='1' and h='0' to make Box fill remaining space */}
        {isLoading && (
          <Flex justify="center" align="center" h="100%">
            {" "}
            {/* 100% of the Box height */}
            <Spinner size="xl" />
          </Flex>
        )}
        {error && (
