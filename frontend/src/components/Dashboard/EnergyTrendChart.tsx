...
const smallFallbackPalette = ["#22c55e", "#64748b", "#fb923c", "#0ea5e9", "#8b5cf6"];
...
const EnergyTrendChart = ({ tenantId, energyDataIds, socDataId }: EnergyTrendChartProps) => {
  ...
  const transformedEnergyData = useMemo(() => {
    ...
    const seriesInfo = energyApiResponse.series.map((s, i) => ({
      name: s.name,
      color: consistentColorMap[s.name] || makePalette(energyApiResponse.series.length)[i],
    }));
  ...
  const transformedSocData = useMemo(() => {
    ...
    const seriesInfo = socApiResponse.series.map((s, i) => ({
      name: s.name,
      color: consistentColorMap[s.name] || makePalette(socApiResponse.series.length)[i],
    }));
  ...
  const transformedCombinedData = useMemo(() => {
    ...
    const seriesInfo = combinedSeriesResponse.series.map((s, i) => ({
      name: s.name,
      color: consistentColorMap[s.name] || makePalette(combinedSeriesResponse.series.length)[i],
    }));
  ...
  return (
    <GridItem
        ...
        {timeRange !== "1D" && timeRange !== "All" && (
          <HStack gap={2}>
            <Text fontSize="sm" fontWeight="medium">
              {timeRange === "1W" && (useCurrentPeriod ? "Current Week" : "Last 7 Days")}
      ...
      <Box flex="1" h="0" minH="300px">
        ...
        {error && <Text color="red.500">Error loading chart data: {error.message}</Text>}
                      ...
                      <AreaChart data={transformedCombinedData!.chartData} syncId="chartSync" margin={{ top: 30, right: 20, left: 0, bottom: 5 }}>
                        <defs>
                          {transformedCombinedData!.seriesInfo.map((series, index) => (
                            <linearGradient key={index} id={`grad_combined_${index}`} x1="0" y1="0" x2="0" y2="1">
                        ...
                        />
                        <YAxis label={{ value: "kWh / %", position: "top", offset: 10 }} />
                    ...
                    <>
                      ...
                      <ResponsiveContainer key="soc-chart" width="100%" height="30%">
                        <AreaChart data={transformedSocData?.chartData || []} syncId="chartSync" margin={{ top: 30, right: 20, left: 0, bottom: 5 }}>
                          ...
                          />
                          <YAxis domain={[0, 100]} label={{ value: "%", position: "top", offset: 10 }} />
                          <CartesianGrid strokeDasharray="3 3" vertical={false} />
                          <Tooltip
            ...
            {timeRange !== "1D" && transformedEnergyData && (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={transformedEnergyData.chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(value: number) => `${value.toFixed(2)} kWh`} labelFormatter={(label) => formatXAxis(label as number)} />
                  <Legend />
                  {transformedEnergyData.seriesInfo.map((series) => (
                    <Bar key={series.name} dataKey={series.name} fill={series.color} isAnimationActive={false} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            )}
