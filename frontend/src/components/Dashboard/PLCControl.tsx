...
const PLCControl = ({ tenantId }: PLCControlProps) => {
  ...
  const { data: serverData, isLoading: loadingControl, error, dataUpdatedAt } = useGetPlcDataControl({ tenantId });

  const { mutate: bulkUpdatePlcDataControl, isPending: isSaving } = useBulkUpdatePlcDataControl({ tenantId });
