...
import { useGetSchedule, useBulkUpdateSchedule } from '@/hooks/useScheduleQueries';
...
const createNewScheduleRowTemplate = (): Omit<ScheduleRow, 'id' | 'updated_at' | 'rec_no'> => ({
  start_time: "00:00:00",
  end_time: null,
  charge_enable: false,
...
const ScheduleControlTable = ({ plantId, tenantDb, date }: ScheduleControlTableProps) => {
  ...
  const sortScheduleRows = useCallback((rows: ScheduleRow[]) => {
  ...
  const validateRows = useCallback((rows: ScheduleRow[]): boolean => {
    ...
    for (const row of rows) {
      ...
      if (timeToMinutes(time) === -1) { // Check for invalid time format first
    ...
    for (const [time, ids] of startTimeCounts.entries()) {
      if (ids.length > 1) { // Found a duplicate start time
  ...
  }, [localData, sortScheduleRows, newRow.start_time]);
  // --- END ---
  ...
  const nextRecNoDisplay = useMemo(() => {
    ...
    const lastUsedRecNo = usedRows.length > 0
      ? Math.max(...usedRows.map(r => r.rec_no))
      : 0;
  ...
  const handleAddRow = () => {
    ...
    if (isNewRowStartTimeInvalid) {
      ...
      if (!isValidFormat) {
        toaster.create({ title: "Invalid Start Time", description: "Please enter a valid start time.", type: "error" });
      } else {
        toaster.create({ title: "Duplicate Start Time", description: "A schedule entry already exists for this start time.", type: "warning" });
      }
      return;
    }
    ...
    const lastUsedRecNo = usedRows.length > 0
      ? Math.max(...usedRows.map(r => r.rec_no))
      : 0; // If no rows are used (only possible if serverData is empty), start at 0
    ...
    if (nextRecNo > 24) {
      toaster.create({ title: "Limit Reached", description: "Cannot add more than 24 schedule entries.", type: "warning" });
      return;
    }
  ...
  const handleSaveAll = () => {
    if (!validateRows(localData)) {
      toaster.create({ title: "Invalid Data", description: "Please fix duplicate start times (highlighted in red).", type: "error" });
      return;
    }
    ...
    bulkUpdateSchedule(dataToSave, {
      onSuccess: (savedData) => { // savedData is the new list from the DB
        toaster.create({ title: "Schedule Saved", description: "All changes saved successfully.", type: "success" });
        // The useGetSchedule hook will be invalidated and refetch
        // serverData, which triggers the useEffect to update localData.
      },
      onError: (err: ApiError) => {
        toaster.create({ title: "Save Failed", description: err.message || "Could not save schedule.", type: "error" });
      }
    });
  };
  ...
  return (
    <VStack gap={4} align="stretch"> {/* Wrap table and button */}
      <Box overflowX="auto">
        <Table.Root size="sm">
          {/* variant="simple"  */}
          ...
          <Table.Body>
              ...
              return (
                ...
                >
                  ...
                  <Table.Cell>
                    ...
                    >
                      <Input
                        ...
                        onChange={(e) => handleChange(row.id, "start_time", (e.target.value || "00:00") + ":00")}
                        readOnly={row.rec_no === 1 && row.start_time === "00:00:00"}
                        bg={(row.rec_no === 1 && row.start_time === "00:00:00") ? "gray.100" : "white"}
                      />
                    </Field.Root>
                  </Table.Cell>
                  <Table.Cell>
                  ...
                  <Table.Cell textAlign="end">
                    <Checkbox.Root variant="outline" checked={row.charge_enable} onCheckedChange={(e) => handleChange(row.id, "charge_enable", e.checked)} >
                  ...
                  <Table.Cell textAlign="end">
                    <Checkbox.Root variant="outline" checked={row.discharge_enable} onCheckedChange={(e) => handleChange(row.id, "discharge_enable", e.checked)} >
                      <Checkbox.HiddenInput />
                      <Checkbox.Control />
                    </Checkbox.Root>
                  </Table.Cell>
                  ...
                  <Table.Cell textAlign="end">
                    <Input type="number" size="sm" w="100px" value={row.charge_power} onChange={(e) => handleChange(row.id, "charge_power", Number(e.target.value))} />
                  </Table.Cell>
                  <Table.Cell textAlign="end">
                    <Input type="number" size="sm" w="100px" value={row.charge_limit} onChange={(e) => handleChange(row.id, "charge_limit", Number(e.target.value))} />
                  </Table.Cell>
                  <Table.Cell textAlign="end">
                    <Input type="number" size="sm" w="100px" value={row.discharge_power} onChange={(e) => handleChange(row.id, "discharge_power", Number(e.target.value))} />
                  </Table.Cell>
            ...
            {nextRecNoDisplay <= 24 && (
              <Table.Row key="new-row" bg="gray.50">
                ...
                <Table.Cell textAlign="end">
                  <Input type="number" size="sm" w="100px" value={newRow.discharge_power} onChange={(e) => handleNewRowChange("discharge_power", Number(e.target.value))} />
                </Table.Cell>
                <Table.Cell>
                  <IconButton
                    aria-label="Add Row"
